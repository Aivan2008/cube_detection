...
def visualize(model, generator, config):
    ...
    for images, y_true, images_only_resized in generator:
        ...
        if count >= nbatches:
            break
    try:
        return np.stack(all_boxes).reshape((-1, 1080, 1280, 3))
    except:
...
def visualize_dt_and_gt(images, y_true, y_pred, config):
    ...
    for i, img in enumerate(images):
        ...
        for j, det_box in enumerate(all_filtered_boxes[i]):
            ...
            cv2.putText(img, config.CLASS_NAMES[all_filtered_classes[i][j]] + " " + str(all_filtered_scores[i][j]) , (det_box[0], det_box[1]), font, 1, (255,10,10), 2, cv2.LINE_AA)
        ...
        for j, gt_box in enumerate(non_zero_boxes):
            ...
            x_scale = 1280/config.IMAGE_WIDTH
            y_scale = 1080/config.IMAGE_HEIGHT
            gt_box[0], gt_box[2] = int(gt_box[0]*x_scale), int(gt_box[2] *x_scale)
            gt_box[1], gt_box[3] = int(gt_box[1] * y_scale), int(gt_box[3] * y_scale)

            #add rectangle and text
            cv2.rectangle(img, (gt_box[0], gt_box[1]), (gt_box[2], gt_box[3]), (0, 255, 0), 2)
            cv2.putText(img, config.CLASS_NAMES[int(non_zero_labels[j])], (gt_box[0], gt_box[1]), font, 1,
